        ...
        <div class="form-grid">
            <label>Role
                <select name="role" required>
                    <option value="mahasiswa" class="text-gray-500" @selected(old('role') === 'mahasiswa')>Mahasiswa</option>
            ...
            <label>NIM Mahasiswa
                <input type="text" class="nim-input" name="nim" value="{{ old('nim') }}" placeholder="Isi jika role mahasiswa">
            </label>
            <label>NIDN Dosen
            ...
            <label>Program Studi
                <select name="program_studi_id" required>
                    ...
                    @foreach ($programStudi as $prodi)
                        <option value="{{ $prodi->id }}" class="text-gray-500" @selected(old('program_studi_id') == $prodi->id)>{{ $prodi->nama }}</option>
                    @endforeach
                </select>
            </label>
            <label>Email
